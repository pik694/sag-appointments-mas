defmodule SagAppointments.Doctor.CoreTest do
  ...
  deffixture appointments() do
    past_day_morning =
      Timex.shift(Timex.today(), days: -7) |> Timex.to_naive_datetime() |> Timex.shift(hours: 9)
    ...
    {[
       ...
       %Appointment{
         id: 3,
         patient: 1,
         slot: Timex.shift(past_day_morning, minutes: 2 * @visit_time)
       }
     ],
     [
       ...
       %Appointment{
         id: 5,
         patient: 2,
         slot: Timex.shift(next_monday_morning, minutes: @visit_time)
       },
       %Appointment{
         id: 6,
         patient: 1,
         slot: Timex.shift(next_monday_morning, minutes: 2 * @visit_time)
       }
     ]}
  end
  ...
  test "can get available slots for period", %{init_state: state, appointments: {_, future}} do
    ...
    available =
      available_slots(state, future, Timex.now(), from: next_monday, until: next_wednesday)
  ...
  test "restricts available slots query to valid period", %{init_state: state} do
    ...
    available =
      available_slots(state, [], next_monday,
        from: Timex.shift(next_monday, days: -200),
        until: Timex.shift(next_monday, days: 200)
      )
